  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-4 rounded-lg border border-slate-200 bg-white p-6">
        ...
        <p className="mt-1 text-sm text-slate-500">{t.buyer ?? "Acheteur non renseigné"} — {tender.source.name}</p>

        <dl className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Départements">{t.departements.join(", ") || "Non renseigné"}</Field>
          <Field label="Budget estimé">{t.budgetEstime ? `${t.budgetEstime.toLocaleString("fr-FR")} €` : "Non renseigné"}</Field>
        ...
        {t.description && (
          <div className="mt-6">
            ...
            <p className="mt-1 whitespace-pre-line text-sm text-slate-600">{t.description}</p>
          </div>
        )}
